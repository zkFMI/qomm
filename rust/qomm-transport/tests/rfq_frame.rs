...
use zkpi_committee::application_crypto::{Signature, SigningKey};
use zkpi_committee::mandate::{Direction, TakerExecutionMandate};
use zkpi_committee::mpc_result::fill_mask_scalar_commitment;
use zkpi_committee::wire::reconstruct;
...
fn cover_has_no_query_but_keeps_both_opened_response_masks_secret() {
    ...
    for (index, value) in values.iter().enumerate() {
        if index != 5 && index != 8 {
            assert_eq!(*value, zkpi_committee::wire::FieldElement::ZERO);
        }
    }
    assert_ne!(values[5], zkpi_committee::wire::FieldElement::ZERO);
    assert_ne!(values[8], zkpi_committee::wire::FieldElement::ZERO);
}

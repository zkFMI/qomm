...
use crate::order::{
    AdmissionTicket, FixedSlotSealer, RandomnessBeacon, ZERO as ZERO_MANIFEST_DIGEST,
};
...
#[derive(Clone)]
struct SlotSealingKeys {
    authority: SigningKey,
    beacon: SigningKey,
    node: SigningKey,
}

impl SlotSealingKeys {
    fn generate() -> Self {
        Self {
        ...
        this.execute(
            "PRAGMA journal_mode=WAL;\
             ...
             CREATE TABLE IF NOT EXISTS slots (\
               ...
               manifest_digest BLOB, batch_digest BLOB);\
...
impl NodeStore {
    ...
    fn submit_frame_request_inner(
        ...
        transaction(&database, |database| {
            ...
            database.execute(&format!(
                "INSERT INTO frames(principal,slot,frame_digest,frame,received_ns) VALUES({},{},{},{},{received_ns})",
                blob(principal.as_bytes()),
                frame.slot,
                blob(&digest),
                blob(raw)
    ...
    fn close_slot_request(
        ...
        transaction(&database, |database| {
            ...
            let frames = database
                .query(&format!(
                    "SELECT hex(principal),hex(frame),received_ns FROM frames WHERE slot={slot} ORDER BY principal"
                ))?
                .into_iter()
                .map(|row| {
            ...
            insert_request(database, principal, request_id, request_digest, &response)?;
            database.execute(&format!(
                "UPDATE slots SET state=1,manifest_digest={},batch_digest={} WHERE slot={slot}",
    ...
    pub fn sealed_execution(&self, slot: u32) -> Result<SealedExecution, String> {
        let database = self.database.lock().expect("node database lock");
        let rows = database.query(&format!(
            "SELECT state,duplicate_count,hex(batch_digest),expected_count FROM slots WHERE slot={slot}"
        ))?;
        let Some(row) = rows.first() else {
            return Err("slot was never opened".into());
        };
        ...
        let rows = database.query(&format!(
            "SELECT hex(sf.frame_digest),hex(f.frame) FROM sealed_frames sf \
...
fn ticket_for_principal(
    ...
    let ticket_id: [u8; 32] = Sha256::new()
        .chain_update(b"QOMM:NODE:PRINCIPAL-TICKET:v1")
...
fn seal_stored_slot(
    ...
    let beacon_value: [u8; 32] = Sha256::new()
        .chain_update(b"QOMM:NODE:CLOSE-BEACON:v1")
        .chain_update(slot.to_be_bytes())
        .chain_update(population_digest(expected))
        .chain_update(deadline_ns.to_be_bytes())
        .finalize()
        .into();
...
impl ResidentNodeServer {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            principals,
            kyb_policy,
            slot_keys: Arc::new(SlotSealingKeys::generate()),
            store,
            registry,
            rate_policy,
            idle_timeout,
...
fn dispatch(
    ...
    match request.get("operation").and_then(Value::as_str) {
        Some("health") => {
            let response = json!({"ok": true, "node": node, "status": "ready", "version": VERSION});

//! Rust port of `scripts/run_bitdec_rounds.py`.
//!
//! MP-SPDZ still compiles and executes the program. When qomm-mpc was linked
...
//! source script's measurement interface.
                ...
                if let (Some(base_rounds), Some(both_rounds), Some(base_mb), Some(both_mb)) = (
                    ...
                    marginal.insert(
                        ...
                        json!({
                            ...
                            "extra_mb": py_round_places(both_mb - base_mb, 3),
                        }),
                    );
                }
            }
...
fn validate(options: &Options) -> HarnessResult<()> {
    ...
    if !options.root.join("compile.py").exists() {
        return Err(format!("{} has no compile.py", options.root.display()).into());
    }
    Ok(())
}

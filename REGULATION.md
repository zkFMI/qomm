...
| Selective disclosure (`rust/defmi/src/viewing.rs`, `rust/qomm-transport/src/binding.rs`) | Opening an individual trade, or one scope of one wallet, to an auditor or a supervisor | Blanket monitoring, which the design cannot provide --- and taking a grant back, which it also cannot |

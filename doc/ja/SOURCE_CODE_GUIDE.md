...
```mermaid
flowchart LR
    U[利用者<br/>匿名KYB資格を提示] -->|固定時刻・固定303バイトを7通| N[7つのQOMMノード]
...
実問い合わせは、銘柄、数量、方向、時刻など最大8個の128ビット値を法 `2^255-19` の加法的秘密分散にする。ノードごとの256バイトpayloadには一つの断片と乱数の詰め物を入れる。問い合わせがない場合も同じ構造のダミー断片を送る。
...
実装: `rust/qomm-demo/`、`qomm_demo/static/`
...
デモ台帳はRust製のメモリ内模型で、保存則と予約超過を検査するが、Avalanche上の正本ではない。本番の口座非開示ノート、型付きzkPI、検証者による証明再検証、状態同期は`qomm-avalanche-vm`と製品受入経路が担う。この実演は研究内容を人が操作して理解するためのもので、本番の認証、監視、負荷分散を備えた取引画面ではない。

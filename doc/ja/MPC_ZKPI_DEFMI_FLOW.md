...
日銀型の国債DVP、共通担保、日中当座貸越については、
[`BOJ_JGB_COLLATERAL_SETTLEMENT.md`](BOJ_JGB_COLLATERAL_SETTLEMENT.md)に、実際の日銀ネットと
DeFMI実装の違い、法人合算予約、国債受領・担保化・貸越・支払の一括更新をまとめた。
...
| 役割別デモとネットワーク可視化 | React Flowで注文者、Maker、7 MPCノード、価格照合、zkPI検証、DeFMI台帳と処理中の経路を表示。資金・在庫は法人モジュールの上限から初期化 |
| 長時間停止・MPCサービス更新をまたぐ期限切れ予約 | 法人所有キューが同じ署名済み要求を再提示し、元のvenueを含む署名とDeFMI正本の予約を照合して解放。現在のMPCサービスIDへの一致は要求しない |
